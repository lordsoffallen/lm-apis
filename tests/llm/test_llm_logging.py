...
from lmapis.logging import LLMLogger
...
class TestLLMLoggingIntegration:
    ...
    def test_llm_with_logger_logs_successful_interactions(
        ...
        with patch('lmapis.llm.get_backend', return_value=mock_backend_class):
            ...
            assert logged_entry.get(
                'error') is None  # error key may not exist if no error
    ...
    def test_llm_continues_when_logging_fails(self, mock_backend_setup):
        ...
        with patch('lmapis.llm.get_backend', return_value=mock_backend_class):
            ...
            assert len(
                failing_backend.saved_entries) == 0  # No entries saved due to failure
    ...
    def test_llm_logs_retry_attempts(self, mock_backend_setup,
                                     llm_logger_with_mock_storage,
                                     mock_storage_backend):
    ...
    def test_llm_logs_with_prefill_response(
        ...
        with patch('lmapis.llm.get_backend', return_value=mock_backend_class):
            ...
            assert any("continue where you left off" in str(msg).lower() for msg in
                       logged_messages)
    ...
    def test_llm_full_call_with_logging(
        ...
        with patch('lmapis.llm.get_backend', return_value=mock_backend_class):
            ...
            with patch(
                'lmapis.llm.Assistant.from_model_response') as mock_from_response:
